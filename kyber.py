...
def get_nearest_integer_to_half(q: int) -> int:
    """
...
def get_coeff_from_eta(eta) -> int:
    """
...
def generate_polynomial(n, q, eta) -> Sequence[int]:
    """
    ...
    :return:
    """
    return [get_coeff_from_eta(eta) % q for _ in range(n)]
...
class KyberSystem:
    ...
    def generate_key_pair(self):
        ...
        for i in range(self.k):
            s_polynomials.append(Polynomial(generate_polynomial(self.n, self.q, self.eta_1)))
        ...
        e_polynomials = []
        for i in range(self.k):
            e_polynomials.append(Polynomial(generate_polynomial(self.n, self.q, self.eta_1)))
    ...
    def encrypt(self, A, t, m: int):
        ...
        r_polynomials = []
        for i in range(self.k):
            r_polynomials.append(Polynomial(generate_polynomial(self.n, self.q, self.eta_1)))
        r = np.array(r_polynomials, ndmin=2).T

        #e1
        e1_polynomials = []
        for i in range(self.k):
            e1_polynomials.append(Polynomial(generate_polynomial(self.n, self.q, self.eta_2)))
        e1 = np.array(e1_polynomials, ndmin=2).T

        #e2
        e2 = Polynomial(generate_polynomial(self.n, self.q, self.eta_2))
    ...
    def denoise(self, coefficient_list: Sequence[int]) -> Sequence[int]:
        """
        ...
        """
        q = self.q
        q_half = get_nearest_integer_to_half(self.q)
    ...
    def encode_message_to_polynomial(self, m: int) -> Polynomial:
        ...
        """
        m_binary_string = decimal_to_binary(m)
        q_half = get_nearest_integer_to_half(self.q)
